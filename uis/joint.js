 ...
 * function using [Totem's modelling framework](/models.view).  This framework typically loads itself as follows:
 *
 ...
 * 
 * Models provides clients an interface to [Totem's workflow engines](/api.view).
 * 
 * Workflow contain a network of systems (aka machines or engines).  A system can be triggered, sampled,
...
var MODEL = { 				// Model parameters
	...
	path: { 				// Paths
		...
		viewer: "/nodetau.view" 		// to model viewer
	},
...
SYSTEM.prototype.open = function () {
	...
	alert( MODEL.path.viewer.tag({ver: q.ver, start:this.name})	);
	alert("subs="+this.chain.length);
	
	var simWindow = window.open(
		MODEL.path.viewer.tag({ver: q.ver, start:this.name}),
		this.name,
...
function content() {    // Render workflow model from the #content(start=MODEL) tag
	BASE.start({	
		parser: MODEL.PARSER
